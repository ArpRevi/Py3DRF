"""
Scene class wrapping a Blender scene.

Scene is the only module in the Py3DRF package that imports bpy (Blender's Python API).
Every other public class (Camera, Mesh, SunLight, Material, PointCloudSettings) is a plain,
backend-agnostic data holder with no bpy dependency. Scene acts as the Blender "backend": it
knows how to translate those Python objects into actual Blender data-blocks, via addCamera()
and addObject().
"""
